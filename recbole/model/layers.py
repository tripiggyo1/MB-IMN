...
class GNN(nn.Module):
    ...
    def __init__(self, embedding_size, step=1):
        ...
        self.w_ih = nn.Linear(self.embedding_size, self.gate_size, bias=True)
        self.w_hh = nn.Linear(self.embedding_size, self.gate_size, bias=True)
        self.b_ah = Parameter(torch.Tensor(self.embedding_size))
        nn.init.normal_(self.b_ah, std=0.02)
        self.linear_edge = nn.Linear(self.embedding_size, self.embedding_size, bias=True)
    ...
    def GNNCell(self, A, hidden):
        ...
        inputs = torch.matmul(A, self.linear_edge(hidden)) + self.b_ah
...
class FuseAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        ...
        self.query_it = nn.Linear(hidden_size, self.all_head_size)
        self.key_it = nn.Linear(hidden_size, self.all_head_size)
        self.value_it = nn.Linear(hidden_size, self.all_head_size)
        self.query_ctx = nn.Linear(hidden_size, self.all_head_size)
        self.key_ctx = nn.Linear(hidden_size, self.all_head_size)
    ...
    def forward(self, input_tensor, ctx_tensor, attention_mask):
        ...
        it_query_layer = self.query_it(input_tensor)
        it_key_layer = self.key_it(input_tensor)
        V = self.value_it(input_tensor)
        ctx_query_layer = self.query_ctx(ctx_tensor)
        ctx_key_layer = self.key_ctx(ctx_tensor)


        it_query_layer = self.transpose_for_scores(it_query_layer)
        it_key_layer = self.transpose_for_scores(it_key_layer)
        V = self.transpose_for_scores(V)
        ctx_query_layer = self.transpose_for_scores(ctx_query_layer)
        ctx_key_layer = self.transpose_for_scores(ctx_key_layer)


        attention_scores_it = torch.matmul(it_query_layer, it_key_layer.transpose(-1, -2))
        attention_scores_ctx = torch.matmul(ctx_query_layer, ctx_key_layer.transpose(-1, -2))

        #Attention Fusion with adding
        attention_scores = attention_scores_it + attention_scores_ctx
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
...
class FuseBasedTransformerLayer(nn.Module):
    """
    One transformer layer consists of a multi-head self-attention layer and a point-wise feed-forward layer.
    ...
    def __init__(
        ...
        super(FuseBasedTransformerLayer, self).__init__()
        self.fuseattetion = FuseAttention(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps
        )
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)

    def forward(self, hidden_states, ctx, attention_mask):
        attention_output = self.fuseattetion(hidden_states, ctx, attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, output_all_encoded_layers=True):  #def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        """
        Args:
...
class ShortTermEncoder(nn.Module):
    ...
    def __init__(
        ...
        layer = FuseBasedTransformerLayer(
            n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])

    def forward(self, hidden_states, ctx, attention_mask, output_all_encoded_layers=True):
        """
        Args:
            hidden_states (torch.Tensor): the input of the TransformerEncoder
            attention_mask (torch.Tensor): the attention mask for the input hidden_states
        ...
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, ctx, attention_mask)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
